...
public:
	...
	void Six_case(Magic_Tiles puzzle); // 6가지의 경우로 보드 검사
	bool same_sol(Magic_Tiles p1, Magic_Tiles p2); // 두 솔루션이 같은 솔루션인지 검사
	~Magic_Tiles();
...
// 두 보드의 솔루션이 같은지 판별
bool Magic_Tiles::same_sol(Magic_Tiles p1, Magic_Tiles p2) {
	for (int k = 0; k < 4; k++) {
		for (int i = 0; i < 9; i++) {
			for (int j = 0; j < 4; j++) {
				if (p1.tileArray[i].numArray[j] != p2.tileArray[i].numArray[j]) {
					solution_count--;
					return false;
				}
			}
